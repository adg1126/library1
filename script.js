...
Book.prototype.markAsRead = function () {
  ...
  [...markAsRead].forEach((btn) => {
    btn.addEventListener('click', function () {
      ...
      this.parentNode.innerHTML = `${library[this.parentNode.id].title} ${
        library[this.parentNode.id].author
      } ${library[this.parentNode.id].pages} ${
        library[this.parentNode.id].status
...
let renderLibrary = (doc) => {
  ...
  let li =
    status == 'Unread'
      ? `<li id=${doc.id}>${title} by ${author} ${pages} pages ${status}<button class='mark-as-read'>Read</button>
...
filterOption.addEventListener('change', function (e) {
  ...
  [...books].forEach((b) => {
    ...
    if (e.target.value == 'all') {
      ...
      bookInfo.includes('Read')
        ? (b.style.display = 'block')
        : (b.style.display = 'none');
    } else if (e.target.value == 'unread') {
      bookInfo.includes('Unread')
        ? (b.style.display = 'block')
        : (b.style.display = 'none');
    }
  });
